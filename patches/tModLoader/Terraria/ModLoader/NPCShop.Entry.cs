		...
		public class Entry
		{
			...
			public sealed class Condition : ICondition
			{
				...
				public static Condition operator !(Condition condition)
				{
					...
					return new Condition(condition.DescriptionText, neg.Compile());
				}
				...
				{
					int x = (int)((Main.screenPosition.X + Main.screenWidth / 2) / 16f);
			...
			public Entry SetPrice(int platinum = 0, int gold = 0, int silver = 0, int copper = 0)
			{
				item.shopCustomPrice = Item.buyPrice(platinum, gold, silver, copper);
				return this;
			}
