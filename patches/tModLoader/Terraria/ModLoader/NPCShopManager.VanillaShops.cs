...
namespace Terraria.ModLoader
{
	public static partial class NPCShopManager
	{
		private static void SetupShop_Zoologist()
		{
			...
			static Condition Completion(float value) => new Condition(NetworkText.FromLiteral("BestiaryCompletion"), _ => Main.GetBestiaryProgressReport().CompletionPercent >= value);
			...
			zoologist.CreateEntry(4830).AddCondition(new Condition(NetworkText.FromLiteral("Dog"), _ => !NPC.boughtDog), Completion(0.25f));
			zoologist.CreateEntry(4910).AddCondition(new Condition(NetworkText.FromLiteral("Bunny"), _ => !NPC.boughtBunny), Completion(0.45f));
			...
			zoologist.CreateEntry(4788).AddCondition(Completion(0.3f), Condition.Hardmode);
		...
		private static void SetupShop_Golfer()
		{
			...
			static Condition GolfScore(int count) => new Condition(NetworkText.FromKey("ShopConditions.GolfScore"), _ => Main.LocalPlayer.golferScoreAccumulated > count);
			...
			golfer.CreateEntry(4264).AddCondition(GolfScore(2000), Condition.DownedSkeletron);
			...
			golfer.CreateEntry(4658).AddCondition(GolfScore(1999), Condition.PhaseFull | Condition.PhaseThreeQuartersAtLeft);
			golfer.CreateEntry(4659).AddCondition(GolfScore(1999), Condition.PhaseHalfAtLeft | Condition.PhaseQuarterAtLeft);
			golfer.CreateEntry(4660).AddCondition(GolfScore(1999), Condition.PhaseEmpty | Condition.PhaseQuarterAtRight);
			golfer.CreateEntry(4661).AddCondition(GolfScore(1999), Condition.PhaseHalfAtRight | Condition.PhaseThreeQuartersAtRight);
		}
		...
		private static void SetupShop_SkeletonMerchant()
		{
			...
			var cond = new Condition(NetworkText.FromLiteral("Time"), _ => Main.time % 60.0 * 60.0 * 6.0 <= 10800.0);
		...
		private static void SetupShop_Mechanic()
		{
			...
			mechanic.CreateEntry(2295).AddCondition(new Condition(NetworkText.FromLiteral("Mechanic's Rod"), _ => NPC.AnyNPCs(369)
		...
		private static void SetupShop_Merchant()
		{
			...
			merchant.CreateEntry(4074).AddCondition(Condition.TimeDay, new Condition(NetworkText.FromLiteral("HappyWind"), _ => Main.IsItAHappyWindyDay));
